...
exports.getLaneTableData=async(req,res) => {
    try {
        ...
        if(getLaneTableData){
            ...
            return Response.customSuccessResponseWithData(res,'Get Facilities Table Data',getLaneTableData,200)
    ...
    try {
        ...
        if (region_id || year || quarter) {
            ...
            if(getLaneEmissionData){
                const data = getLaneEmissionData.map((item) => [item["Facility.name"],item.intensity]);
